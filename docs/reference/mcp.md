...
| `gazetteer` | Reverse-geocode a coordinate to its admin hierarchy (`admin`), name the island(s) containing it (`islands`, when an islands layer is configured), name the mountain range and single mountain it lies in (`mountains.range` / `mountains.mountain`, smallest containing per landform, when a mountains layer is configured), a bearing to the most salient nearby place (`bearing`), and — when a DEM is configured — the terrain exposure (`exposure`: slope + aspect) and the height above sea level (`elevation`). Mirrors `GET /api/v1/gazetteer`: each unit/island/mountain/anchor carries `name_native` + a `name_source` code, a `mountain` also its summit `elevation`, admin units also `local_term` + `equivalent_description`; `exposure` carries `slope_deg`/`slope_percent`, the downslope `aspect_deg`/`aspect_compass` (null/empty when `flat`), `sample_spacing_m` and the DEM `source`; `elevation` carries `meters`, `accuracy_m`/`accuracy_basis`, `vertical_datum`, `sea_level`, `surface_model` and the DEM `source` attribution (distinct from the dataset `license`) — and is **null outside DEM coverage**, since a point beyond the DEM's edge has no known height; `sea_level: true` means the DEM covers the point and holds no value, i.e. water it surveyed; plus a response-wide deduplicated `sources` block and an `available` block saying which optional parts this deployment can answer at all. Only registered when the gazetteer feature is enabled. |
