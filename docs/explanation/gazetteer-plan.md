...
- **M5 — Verification, tuning & eval.** ⏳ *partly blocked on real data.* **Done now:** SRID probe (`VerifySRID`) closing the §4 risk — startup warns if ellipsoidal `Distance` can't resolve EPSG:4326. **Pending the rebuilt GeoPackage (`admin_id`/`parent_id`) + finished sidecar:** calibrate the reach radii / constraint tier against a field gold-set; confirm the boundary constraint end-to-end; make the radii config-injectable once real values are known.
