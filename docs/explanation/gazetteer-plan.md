...
- **M5 — Verification, tuning & eval.** **Validated against the real rebuilt file (2026-07-02).** `VerifySRID` passes (ellipsoidal `Distance` computes correctly despite cosmetic SpatiaLite `unknown SRID` stderr — pre-existing `CastAutomagic` behaviour). `Locate(Würzburg)` returns the full sidecar-enriched chain (Altstadt → county → region → state Bayern → Deutschland); `Bearing` yields correct labels ("16 km E Würzburg", "2 km W Nürnberg", "3.5 km NE Bischofsheim in der Rhön"). Opt-in e2e test in `internal/app` (env-gated, skips in CI). **Open calibration decision:** the strict "most-salient-class-within-reach wins" rule makes a far city (≤60 km) beat a much closer town (e.g. "23 km E Würzburg" while standing ~2 km from Volkach). Tune the radii / add a proximity override once the good-practice tradeoff is decided; radii are already config-injectable candidates.
