...
func (f fakeSampler) ElevationAt(context.Context, domain.Coordinate) (meters float64, ok bool, err error) {
...
func elevMeta() ElevationMeta {
	return ElevationMeta{
		...
		HorizontalM: 6.0, SurfaceModel: "DSM",
	}
}
...
func TestElevationSampled(t *testing.T) {
	...
	svc := NewService(fakeIndex{}, testManifest(), nil, nil, true)
	svc.SetElevationSampler(fakeSampler{meters: 177.5, ok: true, license: lic}, elevMeta())
	...
	if got.VerticalDatum != "EGM2008" || got.AccuracyM != 4.0 || got.HorizontalM != 6.0 || got.SurfaceModel != "DSM" {
		t.Errorf("metadata = %+v, want EGM2008/4/6/DSM", got)
	}
...
func TestElevationSeaLevel(t *testing.T) {
	svc := NewService(fakeIndex{}, testManifest(), nil, nil, true)
	svc.SetElevationSampler(fakeSampler{meters: 0, ok: false}, elevMeta())
...
func TestElevationRejectsNonWGS84(t *testing.T) {
	svc := NewService(fakeIndex{}, testManifest(), nil, nil, true)
	svc.SetElevationSampler(fakeSampler{meters: 100, ok: true}, elevMeta())
