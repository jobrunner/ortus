...
func buildKNNQuery(layer, geom string, hasRtree bool, p domain.Coordinate, k int, maxKM float64, f *output.Filter, filterIndex string) (query string, args []any) {
	...
	//
	// The plan choice is forced in BOTH directions once the ortus attribute index
	// exists (see knnFilterIndexMinKM) — the planner gets a wide radius wrong on
	// its own (full bbox row-fetch before the filter), and on small fixtures it
	// picks the attribute index for narrow radii where the R-tree's tiny bbox
	// wins. Wide radius: attribute-index form — the R-tree moves into an
	// UNCORRELATED IN-subquery, so its box ids are materialized ONCE (an
	// INDEXED BY + JOIN spelling made the planner rescan the whole R-tree per
	// candidate row: 90 s batches) and the driver plans
	// `SEARCH t USING INDEX (col=? AND rowid=?)`. Narrow radius: NOT INDEXED pins
	// the classic R-tree/rowid join. Without the ortus index (nil filter,
	// read-only file, foreign indexes) the query text is unchanged.
	wideFilter := filterIndex != "" && maxKM >= knnFilterIndexMinKM
	if wideFilter {
		fmt.Fprintf(&inner, `SELECT t.*, %s AS %q, AsText(CastAutomagic(t."%s")) FROM "%s" t INDEXED BY %q`,
			distExpr, knnDistColumn, geom, layer, filterIndex)
		args = append(args, p.X, p.Y)
		placeholders := strings.TrimSuffix(strings.Repeat("?,", len(f.Values)), ",")
		fmt.Fprintf(&inner, ` WHERE t."%s" IN (%s)`, f.Column, placeholders)
		args = append(args, f.Values...)
		if hasRtree {
			minX, maxX, minY, maxY := knnBBox(p, maxKM)
			fmt.Fprintf(&inner, ` AND t.rowid IN (SELECT id FROM %q WHERE maxx >= ? AND minx <= ? AND maxy >= ? AND miny <= ?)`,
		...
		fmt.Fprintf(&inner, `SELECT t.*, %s AS %q, AsText(CastAutomagic(t."%s")) FROM "%s" t%s`, distExpr, knnDistColumn, geom, layer, notIndexed)
		args = append(args, p.X, p.Y)
		if hasRtree {
			minX, maxX, minY, maxY := knnBBox(p, maxKM)
