...
# Copy go module files first (better layer caching). The gocog fork is a local
# module referenced via a replace directive, so its go.mod must be present before
# `go mod download` can resolve modules.
COPY go.mod go.sum ./
...
# Since base image 2.0.0 the runtime ships only the SpatiaLite stack (no
# GDAL/Python — spatialite-base-image#6), and a monthly rebuild republishes
# the tags with fresh Alpine patches. CVE fixes arrive via base-image bumps
# (Dependabot) and the monthly rebuild, guarded by the CI Trivy gate and the
# weekly published-image scan.
# Since 3.0.0 PROJ and libspatialite are self-built without libcurl (PROJ CDN
# grid downloads are unavailable) and live under /usr/local/lib — see
# SPATIALITE_LIBRARY_PATH below.
...
# Set SpatiaLite library path for the runtime (self-built since base image
# 3.0.0, installed under /usr/local/lib)
ENV SPATIALITE_LIBRARY_PATH=/usr/local/lib/mod_spatialite.so
...
# Start server
# Configure via ENV: docker run -e ORTUS_SERVER_PORT=9090 -e ORTUS_LOGGING_LEVEL=debug ortus
